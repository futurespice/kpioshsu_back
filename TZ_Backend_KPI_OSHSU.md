...
КПЭ = (П₁ × В₁ + П₂ × В₂ + П₃ × В₃) / 100
где Пₙ — значение показателя, Вₙ — вес показателя.
...
# 14. CORS и переменные окружения
...
# 15. Очерёдность реализации (приоритеты)
...
КПЭ: показатели, значения, базовый расчёт

## Фаза 2 — Важная (Sprint 3-4):
Публикации: CRUD + загрузка файлов-подтверждений
Документы/УМК: загрузка, цепочка согласования
Согласования (Approvals): approve/reject для проректора
Аналитика ректора и проректора
